...
# Creae a logistic regression model
...
cat("From the model's summary we can see that our top three variables identified earlier are all have a high statistical significance. The surprising variable here
    is TotalCharges which also has a high statistical significance but was not featured on the decision tree. To evaluate what information can be gained from TotalCharges
    a third model will be created in which it will feature.")
```
...
```{r Top Four Logistic Regression Text, results='asis', echo=F, include=dataScienceReport}
cat("##Top Four Logistic Regression
Using the top three variables from before, we will now add TotalCharges to the model to see whether it being statistically significant will later contribute to its
accuracy")
```
```{r Top Four Regression, include=dataScienceReport}
# We will start by first making a regression using all variables
topFourLogisticRegressionModel <- createLogisticRegressionModel(formula = Churn~Contract+InternetService+tenure+TotalCharges, dataset = train_df)

# Print a summary of the regression
print("Model Summary")
summary(topFourLogisticRegressionModel)
# Convert Betas into odds ratio
print("Odds Ratios")
exp(coef(topFourLogisticRegressionModel))
```

```{r Model Comparison Text, results='asis', echo=F, include=dataScienceReport}
cat("#Model Comparison
Now that we have create all three models we will test their accuracy against the training dataset and also the test dataset")
```